...
public class AssignLunSimpleHandler extends RemoteAbstractAutomationHandler {
	
	private static Logger logger = LoggerFactory.getLogger(AssignLunSimpleHandler.class);
...
//			otherHbaWwns = this.getHostHbaWwns(hbaWwnOther);
		...
		body.setList("LUN_INFO", this.getLunInfo(deviceParams, storageId, Integer.valueOf(dataDisk)));
...
//		storageDao.batchUpdateLunName(lunList);
	 ...
	 * @throws Exception 
	 */
	private List<Map<String, String>> getLunInfo(Map<String, Object> deviceParams, String storageId, int dataDisk) throws Exception {
		List<Map<String, String>> lunList = new ArrayList<Map<String, String>> ();
		int n = dataDisk / SeConstants.LUN_SIZE; // 数据盘个数
