 ...
 */
public class OpenstackPowerVcCommonHandler extends RemoteAbstractAutomationHandler {

	private static Logger logger = LoggerFactory.getLogger(OpenstackPowerVcCommonHandler.class);
	...
	public String execute(HashMap<String, Object> contenxtParams)
		...
		if(contenxtParams!=null){
			...
			try {
				...
				Class<?> clazz = Class.forName("com.git.cloud.handler.automation.pv.impl." + className);
				...
				logger.error("[OpenstackCommonHandler]执行类["+className+"]异常结束" + e.getCause().getMessage() + commonInfo);
			} finally {
				if (contenxtParams != null)
	...
	public void executeOperate(HashMap<String, Object> reqMap) throws Exception {
		throw new Exception("请重executeOperate方法.");
	};
