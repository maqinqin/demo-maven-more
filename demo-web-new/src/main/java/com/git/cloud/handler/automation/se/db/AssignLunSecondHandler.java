...
public class AssignLunSecondHandler extends RemoteAbstractAutomationHandler {
	private static Logger log = LoggerFactory.getLogger(AssignLunSecondHandler.class);
	...
	protected IDataObject buildRequestData(Map<String, Object> contenxtParmas) throws Exception {
		...
		//获取assign标志
		String assign_sn = (String)devMapInfo.get(StorageConstants.STORAGETWO_FINISHED);
...
//				.get(StorageConstants.RES_POOL_LEVEL);
		...
		sn = devMapInfo.get(StorageConstants.STORAGETWO).toString();
		if(StringUtils.isBlank(sn)){
			throw new Exception("获取"+StorageConstants.STORAGETWO+"信息错误！");
		}
		cur_sn_map.put(StorageConstants.STORAGETWO, sn);
		...
		if (storage_model.equals(StorageConstants.STORAGE_MODEL_VMAX)) {
			...
			for (int i = 0; i < deviceIdList.size(); i++) {
				setHandleResultParam(String.valueOf(deviceIdList.get(i)),
						StorageConstants.STORAGETWO_FINISHED, cur_sn_map.get(StorageConstants.STORAGETWO));
				setHandleResultParam(String.valueOf(deviceIdList.get(i)),
