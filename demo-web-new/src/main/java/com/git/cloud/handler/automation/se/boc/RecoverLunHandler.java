...
public class RecoverLunHandler extends RemoteAbstractAutomationHandler {
	
	private static Logger logger = LoggerFactory.getLogger(RecoverLunHandler.class);
	...
	public String execute(HashMap<String, Object> contenxtParams) {
		...
		if(contenxtParams!=null){
			...
			logger.info("[RecoverLunHandler]分配LUN过程开始" + commonInfo);
			try {
...
//					deviceParams.put(PowerVMAIXVariable.SYSTEMNAME + "_other", deviceParams2.get(PowerVMAIXVariable.SYSTEMNAME));
				...
				logger.info("[RecoverLunHandler]创建Fabric正常结束" + commonInfo);
			} catch(Exception e) {
				rtn_map.put("checkCode", MesgRetCode.ERR_PROCESS_FAILED);
				rtn_map.put("exeMsg", e.getMessage());
				logger.error("[RecoverLunHandler]创建Fabric异常结束," + e + commonInfo);
			} finally {
				if (contenxtParams != null)
	...
	protected IDataObject buildRequestData(Map<String, Object> deviceParams) throws Exception {
		...
		header.setOperation(3029); // 回收LUN
