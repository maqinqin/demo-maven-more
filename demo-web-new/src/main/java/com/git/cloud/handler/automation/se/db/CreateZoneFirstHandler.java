...
public class CreateZoneFirstHandler   extends RemoteAbstractAutomationHandler {

	private static Logger log = LoggerFactory.getLogger(CreateZoneFirstHandler.class);
	...
	private String fabric_key_name = "FABRIC1";;
...
//			coreSwitch = getCoreSwitchName(devMap);
	...
	protected void handleResonpse(Map<String, Object> contenxtParmas,
		...
		for (int i = 0; i < deviceIdList.size(); i++) {
			...
			setHandleResultParam(deviceId, StorageConstants.DB_CRETAE_ZONE_FIRST_FINISHED, "FINISHED");
		}
		result_sb.append("FABRICFINISHED："+"FABRIC1").append("\n");
		if(switch_type.equals(StorageConstants.SWITCH_TYPE_CISCO)){
	...
	public String execute(HashMap<String, Object> contextParams) {
		...
		if(contextParams!=null){
			...
			try {
				...
				//获取创建第一个zone完成标志
				String createFirst_zone_flag = (String)deviceInfoMap.get(StorageConstants.DB_CRETAE_ZONE_FIRST_FINISHED);
				if(StringUtils.isNotBlank(createFirst_zone_flag)){
					throw new Exception("已完成FABRIC1创建zone信息操作！");
				}
				...
				for (int i = 0; i < deviceIdList.size(); i++) {
					String deviceId = String.valueOf(deviceIdList.get(i));
					setHandleResultParam(deviceId, "ZONE1_SCRIPT_FILE", scriptName);
					setHandleResultParam(deviceId, "FABRIC1_CREATE_TIME", create_time_str);
				}
				result_sb.append("ZONE1_SCRIPT_FILE:"+scriptName.toString()).append("\n");
...
//		        updateSwitchSlotStatus();
	...
	private CmSwitchPo getCoreSwitch(Map<String, Object> deviceInfoMap) throws Exception {
		...
		try {
			...
			String errorMsg = "Convert FABRIC1 " + fabricObject + " From JSONObject Failed " + e.toString();
			throw new Exception(errorMsg);
		}
...
//		String coreSwitch = switchPoList.get(0).getSwitchName();
	...
	private List<HashMap<String, Object>> buildCiscoZones(Map<String, Object> contenxtParmas) throws Exception {
		...
		for (int j = 0; j < deviceIdList.size(); j++) {
			...
			Object fabricObject = deviceInfoMap.get(fabric_key_name);
			if (fabricObject == null) {
				String errorMsg = "FABRIC1 do not exist in global param table";
				throw new Exception(errorMsg);
			}
			JSONArray zoneArray = null;
			try {
			...
			for (int i = 0; i < size; i++) {
				...
				if ("".equals(name) || "".equals(fabricName) || "".equals(vsanID) || "".equals(wwpn) || "".equals(pwwn)) {
					String errorMsg = "FABRIC1:" + fabricObject + " Contain Null Value";
					throw new Exception(errorMsg);
				}
