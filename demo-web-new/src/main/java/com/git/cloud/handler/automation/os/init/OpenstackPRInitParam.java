...
public class OpenstackPRInitParam extends LocalAbstractAutomationHandler {

	private static Logger logger = LoggerFactory.getLogger(OpenstackPRInitParam.class);
	...
	public String service(Map<String, Object> contenxtParams) throws Exception {
		...
		logger.info("[OpenstackPRInitParam]构建所需参数实例化开始" + commonInfo);
		//参数清理
		...
		if(len == 0) {
			logger.error("[OpenstackPRInitParam]获取设备信息失败" + commonInfo);
			throw new Exception("[OpenstackPRInitParam]获取设备信息失败" + commonInfo);
		}
		...
		logger.info("[OpenstackPRInitParam]构建所需参数实例化结束" + commonInfo);
	...
	private void buildOpenstackParam(String deviceId, Map<String, Object> contenxtParams, Map<String, String> srvAttrInfoMap, String commonInfo) throws Exception {
		...
		try {
			...
			logger.error("[OpenstackPRInitParam]获取设备信息失败,deviceID:" + deviceId);
		}
		...
		if(openstackVmParamVo == null) {
			logger.error("[OpenstackPRInitParam]获取服务器信息失败,deviceID:" + deviceId);
			throw new Exception("[OpenstackPRInitParam]获取服务器信息失败,deviceID:" + deviceId);
		}
		...
		if(ipList == null|| ipList.size() == 0){
			throw new Exception("[OpenstackPRInitParam]初始化获取IP失败，IP为空");
		}
