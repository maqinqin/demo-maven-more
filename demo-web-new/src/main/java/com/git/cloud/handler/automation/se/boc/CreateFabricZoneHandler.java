...
public class CreateFabricZoneHandler extends RemoteAbstractAutomationHandler {
	...
	protected IDataObject buildRequestData(Map<String, Object> deviceParams)
		...
		String hostHbaWwns = deviceParams.get(PowerVMAIXVariable.SYSTEMNAME) + ":" + hbaWwns;
		if(!"".equals(otherHbaWwns)) {
			hostHbaWwns += ";" + deviceParams.get(PowerVMAIXVariable.SYSTEMNAME + "_other") + ":" + otherHbaWwns;
		}
	...
	private List<Map<String, Object>> getVfinfos(Map<String, Object> deviceParams, String hostHbaWwns) throws Exception {
		...
		for(int i=0 ; i<hostHbaWwnArr.length ; i++) {
			...
			List<String> portList = this.thansArrToList(subArr[1].split(","));
			portList.add(port.getPwwn().toUpperCase().replaceAll(":", ""));
			zoneMap.put("MEMBERS", portList);
			...
			zoneMap = new HashMap<String, Object> ();
			zoneMap.put("NAME", "POC_" + subArr[0] + "_" + port.getFcport());
			portList = this.thansArrToList(subArr[1].split(","));
			portList.add(port.getPwwn().toUpperCase().replaceAll(":", ""));
			zoneMap.put("MEMBERS", portList);
			zoneMap.put("ACTIVE_ZONESET", "TESTING");
			zoneList.add(zoneMap);
	...
	private List<String> thansArrToList(String[] strArr) {
