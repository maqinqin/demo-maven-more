...
public class VmControllerServicePowerVcImpl implements VmControllerService{
	private static Logger logger = LoggerFactory.getLogger(VmControllerServicePowerVcImpl.class);
	@Autowired
	...
	public String vmRunningState(String vmId) throws Exception {
		...
		try {
			...
			runningState = OpenstackPowerVcServiceFactory.getComputeServiceInstance(openstackIp,domainName, token).getServerDetail(targetProjectId, targetVmId, true);
