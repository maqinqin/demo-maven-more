...
_jBoxConfig.languageDefaults = {
    ...
    ok: i18nShow('com_confirm'), /* $.jBox.prompt() 系列方法的“确定”按钮文字 */
    yes: i18nShow('com_Y'), /* $.jBox.warning() 方法的“是”按钮文字 */
    no: i18nShow('com_N'), /* $.jBox.warning() 方法的“否”按钮文字 */
    cancel: i18nShow('com_cancel') /* $.jBox.confirm() 和 $.jBox.warning() 方法的“取消”按钮文字 */
};
...
/**
 * 弹层提示 分三种调用方式：showTip("load");showTip("任意内容");showTip("任意内容",function(){//这里是点击确定后执行的方法});
 * @param tipContent 弹层提示的内容
 * @param confirm_callback confirm框点击确定的回调方法
 */ 
function showTip(tipContent,confirm_callback){
	
  if(tipContent == "load"){
	  jBox.tip("加载中...", 'loading',{top:'15%',opacity: 0,showType:'normal'});
  }
	  ...
	  var submit = function (v, h, f) {
	    ...
	    jBox.confirm(tipContent, i18nShow('com_tip'), submit, { id:'confirm', showType:'slide',showScrolling: false, buttons: { '确定' : true, '取消': false } });
  }	
  else if(tipContent && !confirm_callback){
	  //jBox.messager(tipContent, "提示", 2000, { width: 250, showType: 'slide', icon: 'info' });
	  jBox.alert(tipContent, i18nShow('com_tip'), {opacity: 0.01,top:$(window).height()/2-100,width: 250, showType: 'fade', icon: 'info' ,buttons: { '确定' : true}});
  }''
}

//带回调函数但不带取消按钮的方法
function showTipNoCancel(tipContent,confirm_callback){
 if(tipContent!="load" && confirm_callback){
	  var submit = function (v, h, f) {
	        if (v == true) {
	        	confirm_callback();
	        }
	    ...
	    jBox.confirm(tipContent, i18nShow('com_tip'), submit, { id:'confirm', showType:'slide',showScrolling: false, buttons: {'确定': true }});
  }	
}

function showTipSingleButton(tipContent,confirm_callback){
...
function showError(tipContent){
	$.jBox.error(tipContent, i18nShow('tip_error'),{buttons: { '确定': true}});
}
