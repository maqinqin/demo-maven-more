...
function treeNodeOnClick(e, treeId, node){
	...
	var nextNode = cnode.getNextNode();
	if(node.isParent){
		if(!node.zAsync && node.firstExpand) {
			var nodeTemp = zTree.getNodeByParam("id", nodeId, null);
			zTree.removeChildNodes(nodeTemp);
			expandExecFunc();
			node.firstExpand = false;
		}
...
function addNode(newNodeId,newNodeName,newNodeOpenIcon,newNodeCloseIcon,isSameLevel,bizTypeName,isSelectedAddNode,isParentNode,addToNodeId){	
	...
	if(addToNodeId){
		...
		if(typeof(currentNode) == "undefined"){
			showTip("no node was selected,can not add!");
			return null;
		}
		if(isSameLevel){
 ...
 * @param zTree 节点所在树
 * 
 * @param parentNode 添加的目标节点
 * */
function refreshZTreeAfterAdd(zTree,parentNode){
	//清除当前节点下的子节点再重新加载	
	nodeIcon=null;
	zTree.removeChildNodes(parentNode);
	parentNode.isParent = true;
	parentNode.firstExpand = true;
	treeNodeOnExpand(null,null,parentNode);
	treeNodeOnClick(null, null, parentNode);					
...
function moveTreeNode(targetNodeId,moveNodeId,moveType){
    ...
    if(!targetNode){
    	showTip("target node is not exist !");
    	return null;
    }
    if(!moveNode){
    	showTip("move node is not exist !");
    	return null;
    }
    
    var resultNode = zTree.moveNode(targetNode,moveNode,moveType);
    if(resultNode == null){
    	showTip("移动失败,目标节点和要移动的节点已经是父子关系或试图将两个节点的父子关系互换");
    	return null;
    }else{
    	zTree.selectNode(resultNode,false);
    	treeNodeOnClick(null, null, resultNode);
