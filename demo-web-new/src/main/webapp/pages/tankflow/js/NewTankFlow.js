...
var _left=0;
//节点生成的坐标单元值及拖拽单元值
var initUnit = 1, dragUnit = 1;
var editable = false;
//定义一个区域图类：
function GooFlow(bgDiv,property,referenceType, userComponentDiv, userComponentFrame){
...
//	this.$editable=false;//工作区是否可编辑
	...
	var toolHeight = $(window).height()*0.89-30;
	var toolWidth=0;
	if(property.haveTool){
		this.$bgDiv.append("<div class='GooFlow_tool'"+(property.haveHead? "":" style='margin-top:3px'")+"><div style='min-height:"+470+"px;height:"+toolHeight+"px' class='GooFlow_tool_div'></div></div>");
	...
	switch(this.$title) {
		case "instance":
			this.$bgDiv.append("<div class='GooFlow_work' " +
					"style='width:"+(width+17)+"px;height:"+(height+17)+"px;"
					+(property.haveHead? "":"margin-top:3px")+"'></div>");
			this.$workArea=$("<div class='GooFlow_work_inner' style='width:"+(width+55)+"px;height:"+height+"px'></div>")
		...
		case "designer":
			this.$bgDiv.append("<div class='GooFlow_work' " 
					+"style='width:"+(width+17)+"px;height:"+(height+17)+"px;min-width:720px;min-height:465px;overflow:auto;"
			...
			this.$workArea=$("<div class='GooFlow_work_inner' id='gundongtiao' " +
					"style='width:"+(width-10)+"px;height:"+(height-10)+"px;min-height:465px;min-width:660px;overflow:hidden;'></div>")
				.attr({"unselectable":"on","onselectstart":'return false',"onselect":'document.selection.empty()'});
			this.$bgDiv.children(".GooFlow_work").append(this.$workArea);
			this.$draw=null;//画矢量线条的容器
...
//			debugger
		...
		this.initWorkForNode(userComponentDiv, userComponentFrame);
		...
		this.$lineMove.on("mousedown",{inthis:this},function(e){
			if(e.button==2)
				return false;
			var lm=$(this);
			lm.css({"background-color": e.data.inthis.color.font});
			var This=e.data.inthis;
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		if(this.useSVG !=""){
			...
			this.$workArea.prepend("<div class='GooFlow_work_vml' style='position:relative;width:"+width+"px;height:"+height+"px;min-width:730px;min-height:470px;'></div>");
...
//				debugger;
			...
			// 显示线提示输入框
			This.$textArea.val(oldTxt).css({display:"block",width: 120, height: 20,
				left:t.left+x-This.$workArea[0].parentNode.scrollLeft,
				top: t.top - 10 + y - This.$workArea[0].parentNode.scrollTop}).data("id",This.$focus).focus();
...
//				debugger;
		...
		if(this.color.node) {
			if (json.type.indexOf(" mix") > -1) {
				this.$nodeDom[id].css({"background-color":this.color.mix,"border-color": this.color.mix});
			} else {
				this.$nodeDom[id].css({"background-color":this.color.node,"border-color": this.color.node});
			}
			if (mark && this.color.mark) {
				this.$nodeDom[id].css({"border-color": this.color.mark});
			}
...
//				console.log("click up" + isMoved);
	...
	findSimilar(tmpID, newPosX, newPosY) {
		var horiNo = 0, vertNo = 0;
		var data, horiData = [], vertData = [];
		var similarPX = 8;
		var similarData = [];
...
//				debugger
			...
			/*this.delegate(line, "mouseenter",{inthis:this},function(e) {
				alert(1);
			});*/
...
//			this.$lineDom[id]=GooFlow.prototype.drawLine(id,res.start,res.end,lineData.marked,false,lineData.state);
	...
	setLineType:function(id, newType, M){
		...
		if(newType != "sl"){
			//如果是变成折线
			var res = this.calcPolyPoints(this.$nodeData[from],this.$nodeData[to],this.$lineData[id].type,this.$lineData[id].M, this.$scale);
			if (M) {
	...
	setLineM:function(id, M, noStack){
		...
		var ps=this.calcPolyPoints(this.$nodeData[from],this.$nodeData[to],this.$lineData[id].type,this.$lineData[id].M, this.$scale);
		this.$draw.removeChild(this.$lineDom[id]);
		// 重新画线
		this.$lineDom[id] = this.drawPoly(id, ps.start, ps.m1, ps.m2, ps.end, this.$lineData[id].marked || this.$focus == id);
		this.$draw.appendChild(this.$lineDom[id]);
		if(this.useSVG ==""){
			this.$lineDom[id].childNodes[1].innerHTML=this.$lineData[id].name;
