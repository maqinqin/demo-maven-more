 ...
 * @returns
 */
function startInstance(instanceId, wfInstanceId) {
	return new Promise(function(resolve, reject) {
		$.ajax({
			type : "POST",
			...
			data : {
				...
				"bpmInstanceVo.wfInstanceId" : wfInstanceId
			},
			async : false,
			cache : false,
			success : function(data) {
 ...
 * @returns
 */
function showNodeExecuteLog(instanceId, checkNodeId) {
	return new Promise(function(resolve, reject) {
		$.ajax({
			type : "POST",
			...
			dataType : "json",
			data : {
				"bpmInstanceNodePo.instanceId" : instanceId,
 ...
 * @returns
 */
function nodeSingleExecute(instanceId, wfNodeId, typeCode, nodeType) {
	$.ajax({
		url : ctx + "/workflow/instance/bpmInstance_singleExec.action",
		data : {
			"singleExeVo.instanceId" : instanceId,
			"singleExeVo.wfNodeId" : wfNodeId,
 ...
 * @returns
 */
function getLogResult(laseSize, instanceId) {
	return new Promise(function(resolve, reject) {
		$.ajax({
			type : "POST",
	    ...
	    if(event.button == "2"){ //判断是否是右键
	        ...
	        if(str!==undefined){
				...
				if ("instance" != $('#editType').val()) {
					...
					//查看子流程
					if (nodeObj.type == "sub-process" && checkNodeEditCode != 'none'){
...
function startStateClick() {
	var param = "/pages/tankflow/formPage/globalParamsForm.jsp?"
...
function decisionClick(id) {
	var param = "/pages/tankflow/formPage/decisionForm.jsp?" + "id=" + id
			+ "&state=instance";
...
function forkClick(id) {
	var param = "/pages/tankflow/formPage/forkForm.jsp?" + "id=" + id
			+ "&state=instance";
...
function joinClick(id, nodeId) {
	var param = "/pages/tankflow/formPage/joinForm.jsp?id=" + id + "&nodeId="
...
function containerClick(id, nodeId) {
	var param = "/pages/tankflow/formPage/containerForm.jsp?" + "id=" + id
...
function spClick(id, nodeId) {
	var param = "/pages/tankflow/formPage/subprocessForm.jsp?" + "id=" + id
...
function businessCompClick(nodeObj) {
	...
	if(type=="task"){
		page="/pages/tankflowNew/designer/formPage/oaForm.jsp";
		width=400;
		height=360;
	}else if(type=="cmd"){
		page="/pages/tankflowNew/designer/formPage/commandForm.jsp";
		width=700;
		height=400;
	}else if(type=="scr"){
		page="/pages/tankflowNew/designer/formPage/scriptForm.jsp";
		width=850;
		height=360;
	}else if(type=="api"){
		page="/pages/tankflowNew/designer/formPage/apiForm.jsp";
		width=850;
		height=360;
	}
...
function viewReport() {
	window.location.href = ctx
			+ "/pages/tankflow/instance/instanceReport.jsp?processDefinitionId="
...
function getLogDetails() {
	$.ajax({
		...
		success : function(data) {
			...
			dataStr += "<tr><td>节点名称</td><td>执行策略</td><td>开始时间</td><td>结束时间</td><td>执行状态</td><td>详情</td></tr>";
			var jsonData = data;
			$.each(jsonData , function(i , item){
				...
				dataStr += "<tr><td>" + jsonData[i].nodeName + "</td><td>" + jsonData[i].moduleName + "</td><td>" + beginTime + "</td><td>" + endTime + "</td><td>" + execNodeStatus + "</td>";
...
function showSubProcess(){
	...
	$.ajax({
		...
		success : function(data) {
		  if(data.success){
			 //var url=ctx+"/workflow/instance/bpmInstance_processInstance.action?state=instance&instanceId="+data.msg;
