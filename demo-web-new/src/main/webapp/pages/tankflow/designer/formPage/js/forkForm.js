...
$(document).ready(function() {
	...
	$("#forkForm").validate({
		...
		messages: {"forkName":{"required":"请输入节点名称","maxlength":"字符长度不得大于50","forbidComma":"节点名中不能包含逗号"}},
...
function getTreeNodes(){
	...
	var zNodes = [];
	//获取分支节点数据
	var nodeArray = window.parent.TankFlow.getNextNodes($('#dNodeId').val());
	//准备下拉列表中的内容
	if(nodeArray != null){
...
function getTr(index,routeObj,isDisabled){
	...
	if(isDisabled){
		...
		innerOper = '<td  align="center"><a href="#" onclick="removeRow('+index+')">删除路由</a></td>';
	}
	var inner = '<tr id="exception_'+index+'" align="center">'+
				   '<td><input type="text" class="tf_form_input" style="width:95%;text-align:left;" id="nodeExpression_'+index+'" name="nodeExpression_'+index+'" value="'+expression+'" '+disabled+'/></td>'+
				   '<td><input type="text" class="tf_form_input" style="width:95%;text-align:left;"  id="nodeRoute_'     +index+'" name="nodeRoute_'     +index+'" onclick="showMenu('+index+')" value="'+route+'" '+disabled+'/></td>'+
				   innerOper + '</tr>';
...
function addRules(){
	while(inputArr.length>0){
		...
		if(inputId.indexOf("nodeRoute") != -1){
			$(inputId).rules("add", { required: true, maxlength:50, messages: { required: "请输入路由名称",maxlength:"最大长度50"} });
		}else{
			$(inputId).rules("add", { required: true, maxlength:50, messages: { required: "请输入表达式",maxlength:"最大长度50"} });
		}
	}
}
...
if($('#state').val() == "design" || $('#state').val() == "definition" || 
   ($('#state').val() == "instance" && $('#userAdmin').val() == "userAdmin")){
		...
		//如果分支表达式为空data置为{} ---libin 2016.12.15
		if($('#expressionTable tbody > tr').length > 0) {
			...
			window.parent.TankFlow.onSubmitComponent($('#dNodeId').val(),$('#forkName').val(),JSON.stringify({"routeArray":paramsArr}));
		} else {
			window.parent.TankFlow.onSubmitComponent($('#dNodeId').val(),$('#forkName').val(),JSON.stringify({}));
		}
		cancel();
	}
}
