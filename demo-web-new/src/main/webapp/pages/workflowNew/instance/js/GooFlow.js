...
//	}
   ...
   //  +"<a href='javascript:void(0)' type='mutiselect' class='GooFlow_tool_btn' id='"+this.$id+"_btn_mutiselect'><i class='ico_mutiselect'/></a>"
	...
	this.initDraw("draw_"+this.$id,width*0.98,height*0.95);
	this.$group=null;
	if(property.haveGroup)
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		This.addNode(This.$max,{name:"node_"+This.$max,left:X,top:Y,type:This.$nowType,data:"{}",state:"5"});//添加data属性 libin修改Id
		This.$max++;
	  });
	  //划线或改线时用的绑定
...
//			debugger;
			if(this.$title == "instance"){
				if(typeof(json.state) == 'number' && (json.type == "api" || json.type == "scr" || json.type == "cmd" )){
