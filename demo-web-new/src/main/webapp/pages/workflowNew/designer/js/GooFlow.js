...
//	}
	...
	this.$bgDiv.append("<div class='GooFlow_work' style='height:"+(height)+"px;"+(property.haveHead? "":"margin-top:3px")+"'></div>");
	this.$workArea=$("<div class='GooFlow_work_inner' id='gundongtiao'></div>")
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		//16.11.11加了一个id为graphContainer的滚动条
		This.addNode(This.$max,{name:"node_"+This.$max,left:X+document.getElementById("graphContainer").scrollLeft+document.getElementById("gundongtiao").scrollLeft,top:Y+document.getElementById("gundongtiao").scrollTop+document.getElementById("graphContainer").scrollTop,type:This.$nowType,data:"{}",state:"5"});//添加data属性 libin修改Id
		//王宇加添加节点后自动转为选择按钮
	  ...
	  this.$workArea.mousemove({inthis:this},function(e){
			...
			var ev=mousePosition(e),t=getElCoordinate(this);
			var X,Y;
			X=ev.x-t.left+this.parentNode.scrollLeft+document.getElementById("gundongtiao").scrollLeft;
			Y=ev.y-t.top+this.parentNode.scrollTop+document.getElementById("gundongtiao").scrollTop;
	  ...
	  this.$lineMove.on("mousedown",{inthis:this},function(e){
		  ...
		  var X,Y;
		  X=ev.x-t.left+This.$workArea[0].parentNode.scrollLeft+document.getElementById("gundongtiao").scrollLeft;
		  Y=ev.y-t.top+This.$workArea[0].parentNode.scrollTop+document.getElementById("gundongtiao").scrollTop;
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			defs.appendChild(GooFlow.prototype.getSvgMarker("arrow2_1","green"));
			defs.appendChild(GooFlow.prototype.getSvgMarker("arrow3",GooFlow.prototype.color.mark||"#ff3300"));
			defs.appendChild(GooFlow.prototype.getSvgMarker("arrow3_1","green"));
		}
		else{
...
//		});
		...
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
			...
			tempOldTxt = oldTxt;
			x=(parseInt(from[0],10)+parseInt(to[0],10))/2-60 - document.getElementById("gundongtiao").scrollLeft;
			y=(parseInt(from[1],10)+parseInt(to[1],10))/2-12 - document.getElementById("gundongtiao").scrollTop;
		...
		//滚动监听-隐藏文本域-恢复默认文本
		$("#gundongtiao").bind("scroll",function(){	
				demo.setName(demo.$textArea.data("id"),tempOldTxt,"line");
				demo.$textArea.val("").removeData("id").hide();
				demo.blurItem();
				demo.$textArea.css({display:"none"});
		});
	},
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round")<0){
			...
			if(this.$title == "instance"){
				if(json.state && (json.type == "api" || json.type == "scr" || json.type == "cmd" )){
			...
			if(json.data == "{}" ){
				if(json.type == "start"){
					json.name = i18nShow('qa1');
				}  else if (json.type == "end") {
					json.name = i18nShow('qa2');
				} else if (json.type == "decision") {
					json.name = i18nShow('qa3');
				} else if (json.type == "fork") {
					json.name = i18nShow('qa4');
				} else if (json.type == "join") {
					json.name = i18nShow('qa5');
				} else if (json.type == "cmd") {
					json.name = i18nShow('qa6');
				} else if (json.type == "task") {
					json.name = i18nShow('qa7');
				} else if (json.type == "scr") {
					json.name = i18nShow('qa8');
				} else if (json.type == "api") {
					json.name = i18nShow('qa9');
				}
			}
			this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' name='"+json.name+"' id='"+id+"'nodeType='"+json.type+"' style='top:"+json.top+"px;left:"+json.left+"px' data='"+json.data+"' state='"+json.state+"'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td><td>"+json.name+addi+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
		}
		else{
      ...
      if(mark&&GooFlow.prototype.color.mark){
       ...
       this.$nodeDom[id].addClass("item_mix");
		...
		if(this.$maxWidth < json.left){
			this.$maxWidth = json.left;//libin 获取最大节点
		}
		if(this.$maxHeight < json.top){
			this.$maxHeight = json.top;
		}
	},
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".ico","mousedown",{inthis:this},function(e){
			...
			document.onmousemove=function(e){
				...
				if(X<t.left-This.$workArea[0].parentNode.scrollLeft)
					X=t.left-This.$workArea[0].parentNode.scrollLeft+document.getElementById("gundongtiao").scrollLeft;
				else if(X+This.$workArea[0].parentNode.scrollLeft+This.$nodeData[id].width>t.left+This.$workArea.width())
					X=t.left+This.$workArea.width()-This.$workArea[0].parentNode.scrollLeft-This.$nodeData[id].width+document.getElementById("gundongtiao").scrollLeft;
				if(Y<t.top-This.$workArea[0].parentNode.scrollTop)
					Y=t.top-This.$workArea[0].parentNode.scrollTop+document.getElementById("gundongtiao").scrollTop;
				else if(Y+This.$workArea[0].parentNode.scrollTop+This.$nodeData[id].height>t.top+This.$workArea.height())
					Y=t.top+This.$workArea.height()-This.$workArea[0].parentNode.scrollTop-This.$nodeData[id].height+document.getElementById("gundongtiao").scrollTop;
				This.$ghost.css({left:X+hack+"px",top:Y+hack+"px"});
				isMove=true;
			}
			document.onmouseup=function(e){
				if(isMove)This.moveNode(id,X+This.$workArea[0].parentNode.scrollLeft+document.getElementById("gundongtiao").scrollLeft-t.left,Y+This.$workArea[0].parentNode.scrollTop-t.top+document.getElementById("gundongtiao").scrollTop);
				This.$ghost.empty().hide();
				document.onmousemove=null;
				document.onmouseup=null;
			}
		...
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			...
			switch(type){
				case "start":
					var param = ctx+"/pages/workflowNew/designer/formPage/globalParamsForm.jsp?state=design&id="+id;
					...
					$("#startComponent_div").dialog("option", "title", i18nShow('kea1'));
					break;
				case "end":
...
//					$("#endComponent_div").dialog("option", "title", "结束动作");
					break;
				case "decision":
					var param = ctx+"/pages/workflowNew/designer/formPage/decisionForm.jsp?state=design&id="+id;
					...
					$("#decisionComponent_div").dialog("option", "title", i18nShow('kea3'));
					break;
				case "fork":
					var param = ctx+"/pages/workflowNew/designer/formPage/forkForm.jsp?state=design&id="+id;
					...
					name=
					$("#forkComponent_div").dialog("option", "title", i18nShow('kea2'));									
					break;
				case "join":
//					var param = ctx+"/formPage/joinForm.jsp?state=design&id="+id;
//					$('#joinFrame').attr("src",param);
//					$("#joinComponent_div").dialog({
//						autoOpen : true,
//						modal : true,
//						width : 500
//					});
//					$("#joinComponent_div").dialog("option", "title", "聚合路由定义");
					break;
				default:			
					...
					var param = "?state=design&tempTypeId="+$("#tempTypeId").val()+"&id="+id;
		...
		this.$workArea.delegate(".GooFlow_item > div > div[class!=rs_close]","mousedown",{inthis:this},function(e){
			...
			This.$ghost.css({display:"block",
				width:This.$nodeData[id].width-2+"px", height:This.$nodeData[id].height-2+"px",
				top:This.$nodeData[id].top+t.top-This.$workArea[0].parentNode.scrollTop+hack-document.getElementById("gundongtiao").scrollTop+"px",
				left:This.$nodeData[id].left+t.left-This.$workArea[0].parentNode.scrollLeft-hack+document.getElementById("gundongtiao").scrollLeft+"px",cursor:cursor
			});
			var X,Y;
			X=ev.x-t.left+This.$workArea[0].parentNode.scrollLeft;
			Y=ev.y-t.top+This.$workArea[0].parentNode.scrollTop;
				...
				if(GooFlow.prototype.useSVG!=""){
					if(!this.$lineData[this.$focus].marked){
						if(this.$lineData[this.$focus].state){//libin 如果线是未选中而且state=true
							jq[0].childNodes[1].setAttribute("stroke","green");
							jq[0].childNodes[1].setAttribute("marker-end","url(#arrow2_1)");
						}else {
	...
	focusItem:function(id,bool){
        ...
        jq.css("border-color",GooFlow.prototype.color.line);
			...
			x=(from[0]+to[0])/2-35 - document.getElementById("gundongtiao").scrollLeft;
			y=(from[1]+to[1])/2+6 - document.getElementById("gundongtiao").scrollTop;
	...
	moveNode:function(id,left,top){
		...
		if(left<0)	left=0;
		if(left>3490)left = 3490;
		if(top<0)	top=0;
		if(top>1740)	top=1740;
...
///////////以下为有关画线的方法
	...
	calcPolyPoints:function(n1,n2,type,M){
		...
		if(type=="lr"){
			...
			if(m2[0]>n2.left&&m2[0]<n2.left+n2.width){
				m2[1]=(SP.y>EP.y? n2.top+n2.height+2.5:n2.top);//李斌 修改画线箭头位置
				ep[0]=m2[0];ep[1]=m2[1];
			}
			else{
		...
		//如果是允许中段可上下移动的折线,则参数M为可移动中段线的Y坐标
			...
			if(m2[1]>n2.top&&m2[1]<n2.top+n2.height){
				...
				ep[1]=(m2[1]<n2.top? n2.top:n2.top+n2.height+2);//李斌 修改画线箭头位置
			}
		}
	...
	addLineDom:function(id,lineData){
		...
		if(lineData.type&&lineData.type!="sl")
			...
			res=GooFlow.prototype.calcStartEnd(n1,n2);//计算开始、结束坐标
		if(!res)	return;
