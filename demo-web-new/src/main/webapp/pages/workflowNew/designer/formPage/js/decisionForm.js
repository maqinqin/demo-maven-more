...
$(document).ready(function() {
	...
	$("#decisionForm").validate({
		...
		messages: {"decisionName":{"required":i18nShow('input_node_name'),"maxlength":i18nShow('maxline4'),"forbidComma":i18nShow('node_notice')}},
	...
	//获取分支节点数据，初始化表格
	nodeArray = window.parent.getNextNodes($('#dNodeId').val());
	if(nodeArray != null){
		var inner = '', rowId;
		...
		for(var i = 0 ; i < nodeArray.length ; i++){
			...
			if($('#state').val() == "design"){
				inner ='<tr id="exception_'+rowId+'" align="center">'+
							'<td><input type="text" style="width:100%" id="nodeExpression_'+rowId+'" name="nodeExpression_'+rowId+'"/></td>'+
							'<td><input type="text" style="width:100%" id="nodeRoute_'+rowId+'" name="nodeRoute_'+rowId+'"/></td>'+
							'<td><input type="text" style="width:100%" id="nodeName_'+rowId+'" name="nodeName_'+rowId+'" value="'+nodeArray[i].name+'" disabled/></td>'+
							'<td><input type="hidden" id="nodeId_'+rowId+'" value="'+rowId+'"/></td>'+
						'</tr>';
			}else{
				inner ='<tr id="exception_'+rowId+'" align="center">'+
							'<td><input type="text" style="width:100%" id="nodeExpression_'+rowId+'" name="nodeExpression_'+rowId+'" disabled/></td>'+
							'<td><input type="text" style="width:100%" id="nodeRoute_'+rowId+'" name="nodeRoute_'+rowId+'" disabled/></td>'+
							'<td><input type="text" style="width:100%" id="nodeName_'+rowId+'" name="nodeName_'+rowId+'" value="'+nodeArray[i].nodeName+'" disabled/></td>'+
							'<td><input type="hidden" id="nodeId_'+rowId+'" value="'+rowId+'"/></td>'+
						'</tr>';
			}
...
function onSave(){
	if($('#state').val() == "design"){
		...
		window.parent.onSubmitComponent($('#dNodeId').val(),$('#decisionName').val(),JSON.stringify({"type":$('#decisionType').val(),"nextNodes":paramsArr}));
		cancel();
	}
}
 ...
 */
function cancel(){
	window.parent.closeComponentDialog("decisionComponent_div");
}
